...
public class BaseActivity extends Activity implements Settings {
	...
	public void setCurrentProfile(int profile) {
		if (profile == 0 || profile == getCurrentProfile())
			return;
		Log.i("MyTrace", "ReaderView: " +"Switching from profile " + currentProfile + " to " + profile);
...
//					| View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY
    ...
    {
		Log.i("MyTrace", "BaseActivity: " + "BaseActivity.onCreate() entered");
		...
		Log.i("MyTrace", "BaseActivity: " + "CoolReader version : " + getVersion());
		...
		try {
			...
			if ( fld!=null ) {
				...
				if ( v!=null && v instanceof Integer ) {
					...
					Log.i("MyTrace", "BaseActivity: " + "Screen density detected: " + densityDpi + "DPI");
				}
			}
		} catch ( Exception e ) {
			Log.e("MyTrace", "BaseActivity: " + "Cannot find field densityDpi, using default value");
		}
		...
		Log.i("MyTrace", "BaseActivity: " + "diagonal=" + diagonalInches + "  isSmartphone=" + isSmartphone());
		//Log.i("MyTrace", "BaseActivity: " + "CoolReader.window=" + getWindow());
...
//		setCurrentTheme(theme);
	...
	protected void onPause() {
		Log.i("MyTrace", "BaseActivity: " + "CoolReader.onPause() : saving reader state");
		mIsStarted = false;
		mPaused = true;
...
//	}
	...
	protected void onResume() {
		Log.i("MyTrace", "BaseActivity: " + "CoolReader.onResume()");
		mPaused = false;
		mIsStarted = true;
	...
	public void updateBackground() {
		...
		if (contentView != null) {
			if (bgRes != 0) {
				...
				//Log.v("MyTrace", "BaseActivity: " + "Setting background resource " + d.getIntrinsicWidth() + "x" + d.getIntrinsicHeight());
...
//				getWindow().setBackgroundDrawable(Utils.solidColorDrawable(clBackground));
	...
	public void setCurrentTheme(InterfaceTheme theme) {
		Log.i("MyTrace", "BaseActivity: " + "setCurrentTheme(" + theme + ")");
		currentTheme = theme;
	...
	public void setScreenOrientation( int angle )
	{
		...
		if (newOrientation != screenOrientation) {
			Log.d("MyTrace", "BaseActivity: " + "setScreenOrientation(" + angle + ")");
...
//			Log.d("MyTrace", "BaseActivity: " + "Screen orientation has been changed: ask for change");
...
//			}
    ...
    private void updateBacklightBrightness(float b) {
        ...
        if (wnd != null) {
	    	...
	    	if (b < 0 && b > -0.99999f) {
	    		//Log.d("MyTrace", "BaseActivity: " + "dimming screen by " + (int)((1 + b)*100) + "%");
	    	...
	    	if ( changed ) {
	    		Log.d("MyTrace", "BaseActivity: " + "Window attribute changed: " + attrs);
	    		wnd.setAttributes(attrs);
	    	}
        }
    }
    ...
    private void updateButtonsBrightness(float buttonBrightness) {
        ...
        if (wnd != null) {
	    	...
	    	try {
	    		...
	    		Log.e("MyTrace", "BaseActivity: " + "WindowManager.LayoutParams.buttonBrightness field is not found, cannot turn buttons backlight off");
    	...
    	BackgroundThread.instance().executeGUI(new Runnable() {
			...
			public void run() {
				try {
			    	...
			    	//Log.v("MyTrace", "BaseActivity: " + "Brightness: " + b + ", dim: " + dimmingAlpha);
	...
	private class ScreenBacklightControl {
		...
		public void onUserActivity() {
			...
			if (wl == null) {
				...
				Log.d("MyTrace", "BaseActivity: " + "ScreenBacklightControl: WakeLock created");
			}
			if (!isStarted()) {
				Log.d("MyTrace", "BaseActivity: " + "ScreenBacklightControl: user activity while not started");
				release();
				return;
			}

			if (!isHeld()) {
				Log.d("MyTrace", "BaseActivity: " + "ScreenBacklightControl: acquiring WakeLock");
				wl.acquire();
			}

			if (backlightTimerTask == null) {
				Log.v("MyTrace", "BaseActivity: " + "ScreenBacklightControl: timer task started");
		...
		public void release() {
			if (wl != null && wl.isHeld()) {
				Log.d("MyTrace", "BaseActivity: " + "ScreenBacklightControl: wl.release()");
				wl.release();
			}
...
//				Log.v("MyTrace", "BaseActivity: " + "ScreenBacklightControl: timer task, lastActivityMillis = "
//						+ interval);
				...
				if (interval > screenBacklightDuration) {
					Log.v("MyTrace", "BaseActivity: " + "ScreenBacklightControl: interval is expired");
					release();
				} else {
...
//		}
	...
	public void showToast(String msg, int duration) {
		Log.v("MyTrace", "BaseActivity: " + "showing toast: " + msg);
...
//			ToastView.showToast(getContentView(), msg, Toast.LENGTH_LONG, settings().getInt(ReaderView.PROP_FONT_SIZE, 20));
			Toast toast = Toast.makeText(this, msg, 20);
			toast.show();
		} else {
			// classic Toast
	...
	public void setLanguage(Lang lang) {
		try {
			...
			Log.e("MyTrace", "BaseActivity: " + "error while setting locale " + lang, e);
		}
	}
...
//	}
	...
	public void askConfirmation(int questionResourceId, final Runnable action, final Runnable cancelAction) {
		...
		dlg.setPositiveButton("Ок", new OnClickListener() {
			public void onClick(DialogInterface arg0, int arg1) {
				action.run();
			}
		});
		dlg.setNegativeButton("Отмена", new OnClickListener() {
			public void onClick(DialogInterface arg0, int arg1) {
...
//		if (profile == 0 || profile == getCurrentProfile())
//			return;
//		Log.i("MyTrace", "BaseActivity: " + "Switching from profile " + currentProfile + " to " + profile);
//		mSettingsManager.saveSettings(currentProfile, null);
//		final Properties loadedSettings = mSettingsManager.loadSettings(profile);
//		mSettingsManager.setSettings(loadedSettings, 0, true);
//		currentProfile = profile;
...
//		    public static final int KEYCODE_PAGE_TOPRIGHT = 0x5e; // back
		...
		public Properties loadSettings(BaseActivity activity, File file) {
	        ...
	        if ( file.exists() && !DEBUG_RESET_OPTIONS ) {
	        	try {
	        		...
	        		props.load(is);
	        		Log.v("MyTrace", "BaseActivity: " + "" + props.size() + " settings items loaded from file " + propsFile.getAbsolutePath() );
	        	} catch ( Exception e ) {
	        		Log.e("MyTrace", "BaseActivity: " + "error while reading settings");
	        	}
	        }
		...
		private Properties loadSettings()
		{
	        ...
	        if (existingFile != null) {
				...
				if (dataDir != null) {
					Log.d("MyTrace", "BaseActivity: " + "external settings dir: " + dataDir);
...
//		DelayedExecutor saveSettingsTask = DelayedExecutor.createBackground("saveSettings"); 
		...
		{
			try {
				Log.v("MyTrace", "BaseActivity: " + "saveSettings()");
	    		FileOutputStream os = new FileOutputStream(f);
	    		settings.store(os, "Cool Reader 3 settings");
				Log.i("MyTrace", "BaseActivity: " + "Settings successfully saved to file " + f.getAbsolutePath());
			} catch ( Exception e ) {
				Log.e("MyTrace", "BaseActivity: " + "exception while saving settings", e);
			}
		}
